 ...
 * @param {object} ratings - The ratings object containing the rating counts
 * for each rating.
...
export const calculateAverageRating = (ratings) => {
  const ratingCounts = Object.values(ratings.rating);

  const totalRateCount = ratingCounts.reduce((acc, count) => acc + count, 0);
  const weightedSum = Object.keys(ratings.rating).reduce(
    (acc, rate) => acc + rate * ratings.rating[rate],
    0
  );
